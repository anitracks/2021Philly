...
\DoxyCodeLine{35     \textcolor{comment}{\#endCaps.sub('', csvDF['Case Outcome'][10129]).strip()}}
...
\DoxyCodeLine{85 }
\DoxyCodeLine{86     print(\textcolor{stringliteral}{"{}The column types are:"{}})}
\DoxyCodeLine{87     print(csvDF.dtypes)  \textcolor{comment}{\# print out the data type for each column}}
\DoxyCodeLine{88     \textcolor{comment}{\# calculate the median judgment amount}}
\DoxyCodeLine{89     medJudgment = np.nanmedian(csvDF[\textcolor{stringliteral}{'Judgment Amount'}])}
\DoxyCodeLine{90     print(f\textcolor{stringliteral}{'\(\backslash\)nThe median judgment amount is \$\{medJudgment\}'})}
\DoxyCodeLine{91 }
\DoxyCodeLine{92     \textcolor{comment}{\# need to filter out exact duplicate rows in base dataset}}
\DoxyCodeLine{93     \textcolor{comment}{\# need to look at cases where the same attorney is listed on both sides}}
\DoxyCodeLine{94     \textcolor{comment}{\# try using sent2vec to sort out similar ones}}
\DoxyCodeLine{95     \textcolor{comment}{\# pypi.org/project/sent2vec}}
\DoxyCodeLine{96     outs = \mbox{\hyperlink{namespacephilly_afd61199a26debb52b46c478deea8c038}{outcomeList}}(csvDF)}
\DoxyCodeLine{97     print(\textcolor{stringliteral}{'\(\backslash\)nOutcome Subcategories'})}
\DoxyCodeLine{98     print(f\textcolor{stringliteral}{'\{outs\}'})}
\DoxyCodeLine{99     \textcolor{comment}{\#conn = sqlite3.connect(args.dbfile)  \# connect to the database}}
\DoxyCodeLine{100     \textcolor{comment}{\# load the csvData into the database}}
\DoxyCodeLine{101     \textcolor{comment}{\#csvDF.to\_sql('csvDF', conn, if\_exists='fail', index=False)}}
\DoxyCodeLine{102 }
\DoxyCodeLine{103     stop\_time = datetime.datetime.now()}
\DoxyCodeLine{104     print(f\textcolor{stringliteral}{'\{\_\_file\_\_\} took \{stop\_time-\/start\_time\} seconds'})}
\DoxyCodeLine{105 }
